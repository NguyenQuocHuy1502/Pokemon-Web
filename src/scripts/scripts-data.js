...
function PokemonPage() {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                // Capitalize the first letter for the local database query to match the format
                const capitalizedName = pokemonName.charAt(0).toUpperCase() + pokemonName.slice(1);
                
                // Fetch data from the local backend
                const localRes = await fetch(`http://127.0.0.1:5000/api/pokemon/${capitalizedName}`);
                if (!localRes.ok) {
